...
def classify(dataSet, centrobands, k, img):
    ...
    clalist = calcArgDis(dataSet, centrobands, k, img)#clalist:4w * 20,每个点到20个质心的光谱距
    # 分组并计算新的质心,质心为光谱
...
def kmeans(dataSet, k, img):
    ...
    clalist = calcArgDis(dataSet, centrobands, k, img)
...
class getPUdata():
    def __init__(self):
        ...
        num_cluster = config['num_classes']-1
        ...
        for i in range(num_cluster):
            ...
            cnt = np.zeros(config['num_classes'])
            ...
            Delete = []
            for itercnt in range(len(cnt)):
                if (cnt[itercnt] < 10 and cnt[itercnt] > 0):
                    Remember.append(itercnt)
            for iter_yi in range(len(yi)):
                for iter_Remember in Remember:
                    if yi[iter_yi] == iter_Remember:
                        Delete.append(iter_yi)
                        break
            Delete = np.array(Delete).astype(int)
            sortIndex = np.array(Delete).argsort()
            Delete = Delete[sortIndex[::-1]]
            for iter_Delete in Delete:
                cluster[i].pop(iter_Delete)
                
                yi.pop(iter_Delete)

           
            print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
            train_indices_i, test_indices_i = sklearn.model_selection.train_test_split(cluster[i], train_size=0.04, random_state = 0,stratify = yi)
...
if __name__ == '__main__':
    ...
    args.add_argument('-c', '--config', default=r'config_cifar10.json', type=str,
    ...
    print("Total cluster time : %d", time_sum)
    ...
    io.savemat('D:/Users/d/Desktop/ELR/saveddata.mat', {'cube':cube})  #变量分别保存在名字为xyz下面    
